...
resource "oci_identity_compartment" "domains" {
  ...
  for_each       = {
    for compartment, stage in var.configuration.resident.service.compartments : compartment => stage
...
resource "oci_identity_tag_namespace" "resident" {
  ...
  freeform_tags  = local.freeform_tags
  for_each = {
    for namespace, stage in var.configuration.resident.tenancy.tag_namespaces : namespace => stage
...
resource "oci_identity_tag" "resident" {
  ...
  for_each         = {
    for tag in var.configuration.resident.service.tags : tag.name => tag
...
resource "oci_identity_group" "resident" {
  ...
  for_each       = var.configuration.resident.tenancy.groups
  name           = each.value
...
resource "oci_identity_policy" "domains" {
  ...
  for_each       = var.configuration.resident.service.policies
  compartment_id = oci_identity_compartment.resident.id
...
resource "oci_budget_budget" "resident" {
  ...
  for_each       = {
    for budget in var.configuration.resident.service.budgets : budget.display_name => budget
...
resource "oci_ons_notification_topic" "resident" {
  ...
  compartment_id = oci_identity_compartment.resident.id
  for_each       = var.configuration.resident.service.notifications
...
resource "oci_ons_subscription" "resident" {
  ...
  endpoint       = var.configuration.resident.service.notifications[each.value.name].endpoint
  protocol       = var.configuration.resident.service.notifications[each.value.name].protocol
}
// --- notification service --- //
